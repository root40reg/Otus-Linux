...
nano /etc/pam.d/sshd
```
Ищем и убиваем процесс мешающий установить своё время на ВМ
```
ps -aux |grep VBoxService
kill <id процесса>
```
Смотрим текущую дату, выставляем выходные, снова смотрим через 5 сек., что дата не вернулась на прежнее значение
```
date
date 102823232023  # Суббота
date
```
Далее пробуем зайти в новом окне на ВМ
```
ssh otus@192.168.57.10      # не пустит
ssh otusadm@192.168.57.10   # пустит
```
